...
const Dashboard = ({ onOpenAddSheet, triggerRerender, categories = [] }) => {
  ...
  if (error) {
    return (
      <div className="p-8 text-center flex flex-col items-center justify-center min-h-[70vh]">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-450 mb-6">{error}</p>
        <button
  ...
  return (
    <div className="p-5 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-bold">
            Welcome to Spendly tracker
          </p>
        </div>
        <button
          onClick={() => onOpenAddSheet(null)}
          className="p-3 bg-gradient-to-tr from-primary-600 to-indigo-500 hover:from-primary-500 hover:to-indigo-400 text-white rounded-2xl shadow-lg shadow-primary-500/10 active:scale-95 transition flex items-center justify-center"
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Today Card */}
        <div className="bg-white dark:bg-darkCard p-4.5 rounded-3xl border border-slate-100 dark:border-darkBorder/40 shadow-premium dark:shadow-premium-dark relative overflow-hidden">
          <div className="absolute right-0 top-0 h-16 w-16 bg-primary-500/5 dark:bg-primary-400/5 rounded-bl-full flex items-center justify-center text-primary-600 dark:text-primary-400 pl-4 pb-4">
            <TrendingUp className="h-4.5 w-4.5" />
          </div>
          <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-450 uppercase tracking-wider">
            Today
          </p>
          <h3 className="text-2xl font-black text-slate-900 dark:text-white mt-1.5 flex items-baseline font-sans">
            <span className="text-sm font-semibold mr-0.5">₹</span>
            {(summary.todayTotal || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}
          </h3>
          <span className="text-[10px] font-medium text-slate-450 dark:text-slate-400 mt-1 block">
        ...
        {/* This Month Card */}
        <div className="bg-white dark:bg-darkCard p-4.5 rounded-3xl border border-slate-100 dark:border-darkBorder/40 shadow-premium dark:shadow-premium-dark relative overflow-hidden">
          <div className="absolute right-0 top-0 h-16 w-16 bg-indigo-500/5 dark:bg-indigo-400/5 rounded-bl-full flex items-center justify-center text-indigo-650 dark:text-indigo-400 pl-4 pb-4">
      ...
      {((categoryTotals || []).length > 0) && (
        <div className="bg-white dark:bg-darkCard rounded-3xl p-5 border border-slate-100 dark:border-darkBorder/40 shadow-premium dark:shadow-premium-dark">
          <h3 className="text-xs font-bold text-slate-450 dark:text-slate-300 uppercase tracking-wider mb-4">
            Top Categories
          </h3>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold text-slate-400 dark:text-slate-300 uppercase tracking-wider">
            Recent Spending
          </h3>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar max-w-[60%]">
            <button
              ...
              className={`px-3 py-1 rounded-xl text-[10px] font-bold border transition ${
                selectedCategory === 'All'
                  ...
                  : 'bg-white dark:bg-darkCard border-slate-200/60 dark:border-darkBorder/40 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
              }`}
            >
              All
            </button>
            {(categories || []).slice(0, 3).map((cat) => (
              <button
                ...
                className={`px-3 py-1 rounded-xl text-[10px] font-bold border transition whitespace-nowrap ${
        ...
        {(!filteredExpenses || filteredExpenses.length === 0) ? (
          <div className="bg-white dark:bg-darkCard border border-slate-100 dark:border-darkBorder/40 rounded-3xl p-8 text-center flex flex-col items-center justify-center">
            <div className="h-12 w-12 bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 rounded-2xl flex items-center justify-center mb-3">
              <Tag className="h-5 w-5" />
            </div>
            <p className="text-sm font-bold text-slate-900 dark:text-white">No expenses recorded</p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-xs">
              Tap the '+' button in the bottom navigation to add your first expense!
            </p>
          </div>
        ) : (
          <div className="bg-white dark:bg-darkCard border border-slate-100 dark:border-darkBorder/40 rounded-3xl divide-y divide-slate-100/80 dark:divide-darkBorder/35 shadow-premium dark:shadow-premium-dark overflow-hidden">
            {(filteredExpenses || []).map((expense) => (
              <div
                key={expense._id}
                className="p-4 flex items-center justify-between hover:bg-slate-50/50 dark:hover:bg-slate-900/10 transition"
              >
                <div className="flex items-center gap-3.5 min-w-0">
                  {/* Category icon avatar */}
                  <div
                    className={`h-11 w-11 rounded-2xl flex items-center justify-center text-xl shrink-0 shadow-sm ${getCategoryBgColor(
                      expense.category
                    )}`}
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-bold">
                        {expense.paymentMode}
                      </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className="p-1.5 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-350 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/20 border border-slate-100 dark:border-transparent transition-all"
                      aria-label="Edit"
                    >
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-xl bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-350 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/20 border border-slate-100 dark:border-transparent transition-all"
                      aria-label="Delete"
                    >
